...
function saveInput(e) {
  // auto saves input in local storage
...
function loadInput() {
  // load input from local storage or get default input for the first time user
...
function render() {
  // render input to interactively-readable text
...
function projectAtom(o, p, pl) {
  // recursive function for project
  // o: object, p: parent, pl: level
  let result = {};
  if (o?.constructor?.name === "Object") {
    result.value = []
...
async function linearize(r) {
  // wrapper for linearization recursion
  let lin = await _linearize(r)
  return lin;
}

async function _linearize(r) {
  // linearizing the projected data for rendering
  // r: projected object
  let lin = [];
  if (r?.type === "Object") {
    lin.push({
